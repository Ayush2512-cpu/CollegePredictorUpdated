...
public class Prac9 implements ActionListener{
    ...
    public Prac9(CollegePredictorDAO collegePredictorDAO) {
    ...
    public void actionPerformed(ActionEvent ae)
    {
        if(ae.getActionCommand().equals("PREDICT COLLEGE-->")) {
            ...
            // Store in database
            Student student = new Student(name, phone, email, rank);
            ...
            if (new Validator().check(name, email, phone, rank) & check & pack) {
                ...
                if (radio2.isSelected()) {
                    if (radio3.isSelected())
                        new Page3(b, s, t, fees[fieldName], rank, "General", this.collegePredictorDAO);
                    else if (radio4.isSelected())
                        new Page3(b, s, t, fees[fieldName], rank, "OBC", this.collegePredictorDAO);
                    else
                        new Page3(b, s, t, fees[fieldName], rank, "SC/ST", this.collegePredictorDAO);
                } else if (radio.isSelected()) {
                    if (radio3.isSelected())
                        new Page3m(b, s, t, fees[fieldName], rank, "General", this.collegePredictorDAO);
                    else if (radio4.isSelected())
                        new Page3m(b, s, t, fees[fieldName], rank, "OBC", this.collegePredictorDAO);
                    else
                        new Page3m(b, s, t, fees[fieldName], rank, "SC/ST", this.collegePredictorDAO);
                }
            } else {
        ...
        {
            frame.dispose();
            Page1 page1 = new Page1(this.collegePredictorDAO);
